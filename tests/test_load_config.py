...
def test_invalid_yaml(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    (tmp_path / "config.yaml").write_bytes(b"!!!! broken_yaml")
    monkeypatch.setenv("HOMEBOARD_CONFIG", tmp_path / "config.yaml")
    with pytest.raises(homeboard.config.LoadError):
        homeboard.config.load()

def test_missing_component_config(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
...
def test_component_config_accessible(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    (tmp_path / "config.yaml").write_bytes(b""" \
components:
  dummy_component:
    foo: bar
""")
